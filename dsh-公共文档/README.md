...
The installable package name is `dsh-mobile-remote-companion`. Its DSH bundle starts and stops the Companion with `dsh web`, pins workspace selection to DSH's in-browser directory picker, and stores pairing credentials at `~/.dsh-remote/config.json` by default.
